...
// The bitmap layer holds the image for logo display
  image = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_BOY_SCOUT_LOGO);
  #ifdef PBL_PLATFORM_CHALK
      image_layer = bitmap_layer_create(GRect(20, 38, 60, 77));
  #else
      image_layer = bitmap_layer_create(GRect(1, 38, 60, 77));
  #endif

  bitmap_layer_set_bitmap(image_layer, image);
  bitmap_layer_set_background_color(image_layer, GColorDukeBlue);
